...
app = express();app.use(express.static('./public'));
...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('signin', function(data){ 
    if(userName.indexOf(data)<0){
      var x = {id: socket.id, name: data, available: true}; 
  ...
  socket.on('connectTo', function(data){ 
    if(data.to.available)     
    socket.broadcast.to(data.to.id).emit('isAcceptUser', data.from);   
    else {    
  ...
  socket.on('disconnect', function(){ 
    ...
    for(i=0; i<client_count ; i++){     
      if(clients[i] && clients[i].id==thisid){  
        break;      
      }    
    }   
    clients.splice(i,1);   
